...
$(document).ready(function () {
    ...
    //选中行
    $("#example2 tbody tr").on("click",function(){
        $(this).addClass("pitch");
        $(this).siblings().removeClass("pitch");
        if($(this).hasClass("pitch")){
            // $(this).parent().parent().removeClass("table-hover");
        }
        else {
        }
...
function close() {
    ...
    if(userA.indexOf("Firefox")!=-1||userA.indexOf("Chrome")!=-1){
        window.location.href="about:blank";
    }else{
        window.opener=null;
        window.open("","_self");
...
function closeto() {
    ...
    if(userB.indexOf("Firefox")!=-1||userB.indexOf("Chrome")!=-1){
        window.location.href="about:blank";
    }else{
        window.opener=null;
        window.open("","_self");
...
function changePassword() {
    var changePassword=('<div id="dialog3" style="width: 328px;">\
            <h4>修改密码</h4>\
            <div class="row">\
            <div class="col-xs-12">\
            <div class="condition">\
            ...
            <input id="oldpassword" type="text" placeholder="请输入..." class="form-control"><span class="del fa fa-times"></span>\
            </div>\
            </div>\
            <div class="col-xs-12 choice">\
            <div class="input-group text" style="width: 258px">\
            <div class="input-group-btn seltit" style="width: 93px"><span>新密码</span></div>\
            <input id="newpassword" type="text" placeholder="请输入..." class="form-control"><span class="del fa fa-times"></span>\
            </div>\
            </div>\
            <div class="col-xs-12 choice">\
            <div class="input-group text" style="width: 258px">\
            <div class="input-group-btn seltit" style="width: 93px"><span>确认新密码</span></div>\
            <input id="newpassword2" type="text" placeholder="请输入..." class="form-control"><span class="del fa fa-times"></span>\
            </div>\
            </div>\
            <ul class="col-xs-12 eletcadd">\
            <button id="submit" class="btn btn-warning btn-lg"><i class="fa fa-fw fa-check-square"></i> 提 交</button>\
        <button id="cancel" class="btn btn-danger btn-lg"><i class="fa fa-fw fa-times"></i> 取 消</button>\
        </ul>\
        </form>\
        </div>\
        </div>\
...
$(function() {
    ...
    //自动获取时间天气情况
    $("#date").leoweather({format:'{年}年{月}月{日}号'});
    $("#time").leoweather({format:'{时}:{分}'});
    $("#week").leoweather({format:'星期{周}'});
    $("#icon").leoweather({format:'<img src="img/weather/{图标}.gif"/>'});
    $("#weather").leoweather({format:'{天气}'});
    $("#wind").leoweather({format:'{风向}{风级}级'});
    $("#city").leoweather({format:'{城市}'});
    $("#warm").leoweather({format:'{夜间气温}°C~{气温}°C'});

    // 18:00以后改变天气部分的背景颜色
    var date = new Date();
    if (date.getHours() >= "18") {
        $(".weather").css("background","#246CB1");
        $(".location").css("background","#20609D");
    }

    //检测文本框输入字符后显示删除按钮，点击按钮可清空当前内容
...
(function($) {
    ...
    $.fn.tree = function() {
        ...
        return this.each(function() {
            ...
            var menu1=$(this).siblings().children(".treeview-menu");
            var btn1=$(this).siblings().children("a");
            ...
            btn.click(function(e) {
                ...
                if(isActive){
                    ...
                    if(btn.children().hasClass("fa-angle-down")){
                        menu1.slideUp();
                        btn1.children(".fa-angle-down").first().removeClass("fa-angle-down").addClass("fa-angle-left");
...
(function ($) {
    $.fn.hoverDelay=function (options) {
        ...
        var sets=$.extend(default1,options||{});
        return $(this).each(function () {
            var that=this;
            var hoverTimer,outTimer;
            $(this).hover(function () {
                clearTimeout(outTimer);
                hoverTimer=setTimeout(function () {sets.hoverEvent.apply(that)},
                    sets.hoverDuring
                )
            },function () {
                clearTimeout(outTimer);
