	...
	<div class="wrapper row-offcanvas row-offcanvas-left">
		...
		<section class="content">
			<div class="row">
				<div class="trajectory">
					<div class="trajectory-top">
						<ul class="trajectory-top-ul">
							<li><span>所属单位</span> <select name="" id="danwei">
							</select></li>
							<li><span>姓名</span> <select name="" id="name"></select></li>
							<li><span>开始时间</span><input id="starttime" class="form-control Wdate" placeholder="选择日期" type="text" onClick="WdatePicker()" style="display: inline;width:140px!important;border-radius: 4px!important;"></li>
							<li><span>结束时间</span> <input id="overtime" class="form-control Wdate" placeholder="选择日期" type="text" onClick="WdatePicker()" style="display: inline;width:140px!important;border-radius: 4px!important;"></li>
						</ul>
						
					</div>
					<ul class="trajectory-ul">
						<li><span>采集点数</span> <input id="dianshu" type="text"></li>
					...
					<ul class="trajectory-ul trajectory-ul-sto">
						<li><span>位置：</span> <input id="weizhi" type="text"></li>
						...
						<li style="width: 1000px;overflow: visible;">
				...
				<div id="wcbox" style="margin-bottom: 20px;"></div>
			</div>
		</section>
	</div>
	<!-- 地图 -->
...
<script type="text/javascript" src="http://lbsyun.baidu.com/custom/stylelist.js"></script>
    ...
    <script src="http://webapi.amap.com/maps?v=1.4.3&key=937aaad9648b59955a02cabaa63b35f8"></script>
	...
	var playBtn, pauseBtn, resetBtn, followChk;
		...
		function chaxun(){
			...
			if(start > over && over != "" && start != ""){
				...
				$.ajax({
					type : 'post',
					url : '${path}/peoplespeed/getCoord.do',
					async : true,
					data : {"starttime":start,"overtime":over,"name":devid},
					datatype : 'json',
						...
						if(data!=null){
							...
							if(dian > 0){
								...
								driving.setSearchCompleteCallback(function() { */
									...
									peo = new BMap.Marker(allpoints[0], {icon: new BMap.Icon("${path}/static/img/peo.png", new BMap.Size(35,27), {imageOffset: new BMap.Size(0, 0),anchor : new BMap.Size(10,30)})});
									peo.setLabel(label);
									map.addOverlay(peo);
		...
		function printdiv() {
			...
			window.location.href = "${path}/img/printB.do?center="+
			map.getCenter().lng+","+map.getCenter().lat
			+"&zoom="+map.getZoom()+"&allpolins="+allpolins
			+"&filename="+filename;
		}
		...
		$(function(){
			...
			//获取单位
			 $.ajax({
				type : 'post',
				url : '${path}/peoplespeed/getCompany.do',
				async : false,
				data : {},
				datatype : 'json',
				success : function(data){
					if(data != null) {
						$.each(data,function(n,v){
							$("#danwei").append("<option onclick='chufa()'>"+v.uuname+"</option>");
						});
					}
				}
			}); 
		...
		function chufa(){
			...
			$.ajax({
				type : 'post',
				url : '${path}/peoplespeed/getName.do',
				async : true,
				...
				success : function (data) {
					if (data != null) {
						...
						$("#name").empty();
						$.each(data,function(n,v){
							$("#name").append("<option value='"+v.name+"'>"+v.name+"</option>");
						});
					};
				}
			});
