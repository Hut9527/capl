...
public class MonthlyController {
	...
	public UserCarAttendanceVo selectByTime(AttendanceTimeVo time) {
		...
		//调整样式
		int l = time.getMonthly().length();
		String ym = "";
		if(l == 1) {
			ym = time.getYear() + ".0" + time.getMonthly();
		} else {
			ym = time.getYear() + "." + time.getMonthly();
		}
		time.setYearmonthly(ym);
		time.setYear("");
		time.setMonthly("");
		...
		//计算整理数据
		UserCarAttendanceVo count = AttendanceUtils.count(car, user,30,
