...
public class GaodePointUtils {
 ...
 public static boolean outOfChina(double lat, double lon) {
	...
	/*public double[] transLatLng(double wgLat, double wgLng) {
	...
	private double transLat(double x, double y, double pi) {
		double ret;
	...
	private double transLng(double x, double y, double pi) {
		double ret = 300.0 + x + 2.0 * y + 0.1 * x * x + 0.1 * x * y + 0.1
				* Math.sqrt(Math.abs(x));
		ret += (20.0 * Math.sin(6.0 * x * pi) + 20.0 * Math.sin(2.0 * x * pi)) * 2.0 / 3.0;
