...
function aa(e,fun){
	...
	if(len == 9){
		$('#lng5').val(e.point.lng);
		$('#lat5').val(e.point.lat);
		bo1 = true;
		var allOver = map.getOverlays();
		map.removeOverlay(allOver[i2]);
		var polygon = new BMap.Polygon([
		                        		new BMap.Point($('#lng1').val(), $('#lat1').val()),
		                        		new BMap.Point($('#lng2').val(), $('#lat2').val()),
		                        		new BMap.Point($('#lng3').val(), $('#lat3').val()),
		                        		new BMap.Point($('#lng4').val(), $('#lat4').val()),
		                        		new BMap.Point($('#lng5').val(), $('#lat5').val()),
		                        		], {strokeColor:"blue", strokeWeight:2, strokeOpacity:0.5});
		map.addOverlay(polygon); 
	}
	if(len == 10){
		$('#lng6').val(e.point.lng);
		$('#lat6').val(e.point.lat);
		bo1 = true;
		var allOver = map.getOverlays();
		map.removeOverlay(allOver[i3]);
		var polygon = new BMap.Polygon([
		                        		new BMap.Point($('#lng1').val(), $('#lat1').val()),
		                        		new BMap.Point($('#lng2').val(), $('#lat2').val()),
		                        		new BMap.Point($('#lng3').val(), $('#lat3').val()),
		                        		new BMap.Point($('#lng4').val(), $('#lat4').val()),
		                        		new BMap.Point($('#lng5').val(), $('#lat5').val()),
		                        		new BMap.Point($('#lng6').val(), $('#lat6').val()),
		                        		], {strokeColor:"blue", strokeWeight:2, strokeOpacity:0.5});
		map.addOverlay(polygon); 
